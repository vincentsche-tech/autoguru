// ListKraft — listing core (runtime-agnostic)
...
// Compact-format a list of fitment rows into one clean Gemini-style string.
// Algorithm: greedy single-link clustering on common-token-prefix (>= 2
// tokens), then per cluster emit "<BaseModel> [(variants)] (<yearStart>-<yearEnd>)".
// Falls back to `rows.join("; ")` when any row fails to parse (so unparseable
// data still surfaces -- never silently dropped).
//
//   Input:  ["2010-2014 Ranger 400 HO", "2012-2013 Ranger 400",
//            "2012-2013 Ranger 400 Midsize", "2009-2013 Ranger 500", ...]
//   Output: "Ranger 400 (HO, Midsize) (2010-2014)\nRanger 500 (Crew Midsize) (2009-2013)\n..."
//
// Why: Gemini naturally collapses "Ranger 500" + "Ranger 500 Crew Midsize" into
// one line and merges their year range. The original "; "-join dumped every
// raw row (often 20-40 lines) which buyers cannot scan. Mirrored in
// server.py::_format_fitment so JS lib and Python server stay byte-identical.
export function formatFitment(rows) {
  if (!Array.isArray(rows)) return String(rows || "-");
  const safe = rows.map((r) => String(r || "").trim()).filter(Boolean);
  if (safe.length === 0) return "-";
  if (safe.length === 1) return safe[0];
  const YR_RE = /^\s*\(?(?:(\d{4})\s*[-–—]\s*(\d{2,4})|(\d{4}))\)?\s+(.+?)\s*$/;
  const YR_RE_BACK = /^(.+?)\s*\(?(?:(\d{4})\s*[-–—]\s*(\d{2,4})|(\d{4}))\)?\s*$/;
  ...
  const YR_RE_MID = /^(.+?)\s*\(?(?:(\d{4})\s*[-–—]\s*(\d{2,4})|(\d{4}))\)?\s+(.+?)\s*$/;
  const parsed = [];
  for (const row of safe) {
    const mFront = row.match(YR_RE);
    const mBack = !mFront ? row.match(YR_RE_BACK) : null;
    const mMid = !mFront && !mBack ? row.match(YR_RE_MID) : null;
    const m = mFront || mBack || mMid;
    if (!m) { parsed.push({ ok: false, raw: row }); continue; }
    let y1, y2, body;
    if (mFront) {
      if (m[3]) { y1 = y2 = +m[3]; }
      else { y1 = +m[1]; y2 = +m[2]; if (m[2].length === 2) y2 = 2000 + y2; }
      body = m[4].trim();
    } else if (mBack) {
      if (m[4]) { y1 = y2 = +m[4]; }
      else { y1 = +m[2]; y2 = +m[3]; if (m[3].length === 2) y2 = 2000 + y2; }
      body = m[1].trim();
    } else {
      // middle-year: m[1] = Make/Model, m[2..3] = year-range or m[4] = single year,
      // m[5] = spec (engine/body/side). Reassemble so Make/Model stays at the
      // front of the final "<body> (<year>)" output.
      if (m[4]) { y1 = y2 = +m[4]; }
      else { y1 = +m[2]; y2 = +m[3]; if (m[3].length === 2) y2 = 2000 + y2; }
      body = (m[1] + " " + m[5]).trim();
    }
    parsed.push({ ok: true, yearStart: y1, yearEnd: y2, body });
  }
  // Fallback: any unparseable row -> keep the raw "; "-joined dump so no
  // data is silently lost.
  if (parsed.some((p) => !p.ok)) return safe.join("; ");
  // De-dup by body (case-insensitive), keep first occurrence.
  const seen = new Set();
  const uniq = [];
  for (const p of parsed) {
    const k = p.body.toLowerCase();
    if (seen.has(k)) continue;
    seen.add(k);
    uniq.push(p);
  }
  uniq.sort(
    (a, b) => a.yearStart - b.yearStart || a.body.localeCompare(b.body)
  );
  // One clean line per vehicle: "<Full Model> (<yearStart>-<yearEnd>)".
  // Handles both "2016-2020 Mercedes-Benz GLC 300" and "Hyundai Accent 2012-2019".
  return uniq
    .map((p) => {
      const yr =
        p.yearStart === p.yearEnd ? String(p.yearStart) : p.yearStart + "-" + p.yearEnd;
      return p.body + " (" + yr + ")";
    })
    .join("\n");
}
...
export function extractPkgFitment(pkgText) {
  ...
  for (const m of String(pkgText).matchAll(FIT_CUE_RE)) {
    ...
    // we just fed the cue line to fitmentLines(), it stripped everything
    // before the year range, losing the Make/Model. Mirrored in server.py
    // (search "Acura/Honda Accord steering-knuckle regression").
    const yCue = line.match(FIT_YEAR_RE);
    const makeModel = yCue
      ? line
          .slice(0, yCue.index)
          .trim()
          .split(/\s+/)
          .filter((w) => /^[A-Z][A-Za-z0-9-]+$/.test(w))
          .slice(0, 3)
          .join(" ")
      : "";
    ...
    if (cleaned.length) {
      for (const r of cleaned) {
        // If the row lost the Make/Model during prose split (Acura/Honda
        // Accord steering-knuckle regression Sept 2026), prepend it.
        if (
          makeModel &&
          !r.toLowerCase().includes(makeModel.toLowerCase())
        ) {
          const yi = r.match(FIT_YEAR_RE);
          if (yi) {
            // Insert Make/Model right BEFORE the year range so the
            // downstream YR_RE_BACK / formatFitment parser sees "<Year> <Body>".
            push(
              `${makeModel} ${r.slice(yi.index, yi.index + yi[0].length)}${r.slice(
                yi.index + yi[0].length
              )}`
            );
            continue;
        }
        }
        // Legacy bare-year recovery path (kept for compact 1-line cues like
        // "Fits for BMW Z3 1996-2002" where fitmentLines returned the year
        // range alone).
...
// When a supplier data package's FIRST line is already a real eBay-style
// product title (Part Type + Make/Model + part number), we NORMALIZE it into
// the listing title instead of letting the model invent a generic
// "Auto Part for …" placeholder. These helpers power that safety net inside
// buildResult(). They never invent part numbers — verifyOutput() still guards
// the final output via the whitelist.
const TITLE_MAKE_RE =
  /\b(Polaris|Honda|Yamaha|Kawasaki|Suzuki|BMW|Mercedes[-\s]?Benz|Mercedes|Toyota|Nissan|Jeep|Dodge|GMC|Chevrolet|Chevy|Subaru|Audi|Volkswagen|VW|Hyundai|Kia|Mazda|Lexus|Acura|Chrysler|Ram|Tesla|Can-?Am|Arctic\s*Cat|Arctic|Textron|CFMOTO|Hisun|Sea-?Doo|Ski-?Doo|Bobcat|John\s*Deere|Caterpillar|Cat|Kubota|Husqvarna|Stihl)\b/i;

// True when `t` looks like a genuine product title rather than a listing
// header / spec-only line. Gate: must name the part type OR carry a part
// number together with a vehicle make. Bare first lines like
// "Package Contents:" or "OEM Part Number: 1332731" are rejected.
function looksLikeProductTitle(t) {
  const s = (t || "").trim();
  if (s.length < 10 || s.length > 120) return false;
  if (looksLikeEcho(s)) return false;
  if (isCategoryPath(s)) return false;
  const hasPartType = PART_TYPE_KEYWORDS.some(([re]) => re.test(s));
  const hasPartNo = /#?\b[A-Z0-9-]{4,}\b/.test(s);
  const hasMake = TITLE_MAKE_RE.test(s);
  ...
  // A bare "1x Clutch Disc" / "1x Front Differential" packing line has a part
  // type but no make/year/number, so it is rejected and the deterministic
  // fallbackTitle() synthesises the real "<PartType> for <Year Make>" title.
  if (hasPartNo) return true;
  if (hasPartType && (hasMake || hasYear)) return true;
  return false;
}

// Normalise a package product-title line into a clean <=80-char eBay title.
// Drops fluff like "Replacement"/"Premium" and trims from the end at a word
// boundary if still over budget — never cutting before an OEM number token.
function normalizePkgTitle(t) {
  let s = (t || "").trim().replace(/\s+/g, " ");
  s = s.replace(/^(?:high[-\s]?quality\s+|premium\s+|new\s+|aftermarket\s+|genuine\s+|oem\s+|replacement\s+for\s+)/i, "");
  s = s.replace(/\s*\breplacement\b\s*/i, " ").trim().replace(/\s+/g, " ");
  if (s.length > 80) {
    const words = s.split(" ");
    const keep = [];
    let len = 0;
    for (const w of words) {
      const add = (keep.length ? 1 : 0) + w.length;
      if (len + add > 80) break;
      keep.push(w);
      len += add;
    }
...
// A "weak" title adds no real info — a generic placeholder the model emits
// when it couldn't find specifics. Such titles must be replaced by the
// normalised package title rather than shipped to the seller.
function isWeakTitle(t) {
  const s = (t || "").trim();
  if (!s) return true;
  if (/^auto\s*part\b/i.test(s)) return true;
  if (/^generic\b/i.test(s)) return true;
  if (/^replacement\s+part\b/i.test(s)) return true;
  const hasPartNo = /#?\b[A-Z0-9-]{4,}\b/.test(s);
  const hasPartType = PART_TYPE_KEYWORDS.some(([re]) => re.test(s));
  const hasYear = /\b(?:19|20)\d{2}\b/.test(s);
  const hasMake = TITLE_MAKE_RE.test(s);
  if (!hasPartNo && !hasPartType && !(hasYear && hasMake)) return true;
  return false;
}

// Token-overlap between two titles (fraction of a's tokens present in b).
// Used to decide whether a model title already incorporates the package's
// key tokens — if so we don't prepend the normalised package title (avoids a
// duplicate-looking pair and never clobbers good model work).
...
const CATEGORY_RULES = [
  ...
  [/\bdifferential(s)?\b/, "eBay Motors > Parts & Accessories > Car & Truck Parts & Accessories > Suspension & Steering > Differentials & Parts"],
  ...
  [/\bpower\s+steering\s+pump(s)?\b/, "eBay Motors > Parts & Accessories > Car & Truck Parts & Accessories > Steering & Suspension > Power Steering Pumps"],
  // Engine & drivetrain
...
const PART_TYPE_KEYWORDS = [
  [/\bdifferential(s)?\b/i, "Differential"],
  ...
  [/\bpower\s+steering\s+pump\b/i, "Power Steering Pump"],
...
// Premium, eBay-compliant listing HTML (no JS / no external links / inline
// <style> / responsive). Built deterministically from real engine fields —
// never from model-invented marketing prose (hallucination red line).
const HTML_STYLE = `.ebay-container{font-family:'Segoe UI',Roboto,Helvetica,Arial,sans-serif;color:#222;line-height:1.6;max-width:1000px;margin:0 auto;padding:15px;background-color:#f8f9fa}
...
export function buildHtml(title, fitment, bullets, desc, specs, pkgIncludes) {
  const specVal = (re) => {
    const hit = (specs || []).find(([k]) => re.test(k || ""));
    return hit ? hit[1] : "";
  };
  // Subtitle from real specifics (Placement / Material / Fitment Type / Finish).
  const subParts = [];
  for (const key of ["Placement on Vehicle", "Material", "Surface Finish", "Fitment Type"]) {
    const v = specVal(new RegExp(key, "i"));
    if (v && !/does not apply/i.test(v)) subParts.push(v);
    if (subParts.length >= 3) break;
  }
  const subtitle = subParts.length ? subParts.join(" | ") : "Direct Replacement Part";

  // Badges: derived from real data + one seller promise (no part-number claims).
  const badges = [];
  if (fitment && fitment !== "-") badges.push("\u2699\uFE0F OE Spec Fitment");
  const pkQty = specVal(/package quantity/i);
  const isPair =
    (pkgIncludes || []).length >= 2 ||
    /\bpair\b|2[-\s]?pc|2x|\bset\b/i.test(title || "") ||
    /\bpair\b|2[-\s]?pc|2x/i.test(pkQty);
  if (isPair) badges.push("\uD83D\uDCE6 Pair (Left + Right)");
  const warranty = specVal(/warranty/i);
  if (warranty && !/does not apply/i.test(warranty)) badges.push("\uD83D\uDEE1\uFE0F " + warranty);
  badges.push("\uD83D\uDE9A Fast & Free Shipping");

  // Fitment rows -> <li> with Make/Model bolded at the front.
  const fitRows = String(fitment || "")
    .split(/\n+/)
    .map((s) => s.trim())
    .filter(Boolean);
  const boldMakeModel = (line) => {
    const m = line.match(/^([A-Z][A-Za-z0-9-]*(?:\s+[A-Z][A-Za-z0-9-]*){0,1})/);
    if (!m) return esc(line);
    const lead = m[1];
    if (/^(L4|V6|V8|Front|Rear|Left|Right|Driver|Passenger|Petrol|Diesel|Coupe|Sedan|Hatchback|Convertible|Wagon|SUV|Truck)$/i.test(lead))
      return esc(line);
    return `<strong>${esc(lead)}</strong>` + esc(line.slice(lead.length));
  };

  const p = [];
  p.push('<meta name="viewport" content="width=device-width, initial-scale=1.0">');
  p.push(`<style>${HTML_STYLE}</style>`);
  p.push('<div class="ebay-container">');
  p.push(
    `  <div class="ebay-header"><h1>${esc(title || "Product Listing")}</h1><p>${esc(subtitle)}</p></div>`
  );
  p.push('  <div class="ebay-card">');
  p.push(
    `    <div class="badge-container">${badges
      .map((b) => `<span class="badge">${esc(b)}</span>`)
      .join("")}</div>`
  );
  if (desc) p.push(`    <p class="ebay-lead">${esc(desc)}</p>`);
  p.push("  </div>");
  if (fitRows.length) {
    p.push('  <div class="ebay-card">');
    p.push('    <h2 class="section-title">Vehicle Compatibility</h2>');
    p.push(
      '    <p class="section-hint">Please confirm your vehicle\'s year, model and engine before ordering.</p>'
    );
    p.push(
      `    <ul class="fitment-list">${fitRows
        .map((r) => `<li>${boldMakeModel(r)}</li>`)
        .join("")}</ul>`
    );
    p.push("  </div>");
  }
  if (specs && specs.length) {
    const rows = specs
      ...
      .join("\n");
    p.push('  <div class="ebay-card">');
    p.push('    <h2 class="section-title">Product Specifications</h2>');
    p.push(`    <table class="spec-table">\n${rows}\n    </table>`);
    p.push("  </div>");
  }
  if (bullets && bullets.length) {
    p.push('  <div class="ebay-card">');
    p.push('    <h2 class="section-title">Why Choose This Part?</h2>');
    p.push(
  ...
  if (pkgIncludes && pkgIncludes.length) {
    p.push('  <div class="ebay-card">');
    p.push('    <h2 class="section-title">Package Includes</h2>');
    p.push(
      `    <ul class="pkg-list">${pkgIncludes
        .slice(0, 6)
        .map((i) => `<li>${esc(i)}</li>`)
        .join("")}</ul>`
    );
  ...
  p.push(
    '  <div class="note-box"><strong>\u26A0\uFE0F Professional Installation Recommended:</strong> Suspension and steering components are vital to driving safety. A certified technician installation and a post-installation wheel alignment are strongly recommended. Please match the original OE part number before ordering.</div>'
  );
  p.push("</div>");
  return p.join("\n");
}
...
1. Three eBay titles (max 80 chars each), one per line as "* <title text>". PRIORITY: if the DATA PACKAGE Title line above already reads like a real eBay title (it contains a Part Type + Make/Model + part number), treat it as the source of truth — NORMALIZE it (trim to <=80 chars, keep Part Type + OEM number + Make/Model; drop filler words like "Replacement" only if you need room), and do NOT invent a new generic title. Never output a generic "Auto Part for …" title when the package gives a specific product. Include Part Type + OEM/Interchange + Fitment + side/placement when present. Title Case, no decorative punctuation (! @ ?). Avoid unclear abbreviations.
...
3. Fitment. One vehicle line per "* " entry: "* <Year-Range> <Make> <Model> [Trim] [engine] [body] [Side]". CRITICAL: the Make and Model are NEVER optional — they MUST appear at the FRONT of every line, even when several lines share the same Make/Model. If the data package lists multiple trims/engines/body styles for the SAME Make/Model, output one line per distinct (year+engine+body+side) combination and KEEP the Make/Model on every line. WRONG (Make/Model collapsed away — buyer cannot tell which vehicle): "* 2003-2007 l4 2.4L Petrol Coupe Front Left & Right". RIGHT (Make/Model at front): "* 2003-2007 Honda Accord l4 2.4L Petrol Coupe Front Left & Right". Every distinct fitment from the package goes on its own line — never merge into one prose sentence.
...
Do NOT invent any OEM numbers, interchange numbers or vehicle fitments not listed above. Every part number you output MUST exist verbatim in the data package. Never add "common" interchange numbers from your own knowledge. When the DATA PACKAGE Title line is already a specific product title, NORMALIZE it into your titles — never replace it with a generic "Auto Part for …" title.
...
export function buildResult(pkgText, llmText, model, seconds, tokens) {
  ...
  // generic "Auto Part for …" placeholders. Output still passes verifyOutput().
  const pkgTitleNorm = looksLikeProductTitle(title) ? normalizePkgTitle(title) : "";
  if (pkgTitleNorm) {
    const modelStrong = titles.find((t) => !isWeakTitle(t.text));
    if (!modelStrong) {
      titles = [{ text: pkgTitleNorm, len: pkgTitleNorm.length }, ...titles];
    } else if (titleOverlap(modelStrong.text, pkgTitleNorm) < 0.5 && isValidTitle(pkgTitleNorm)) {
      titles = [{ text: pkgTitleNorm, len: pkgTitleNorm.length }, ...titles];
    }
  }
  ...
  // Mirrored in server.py (search "P0 fix (Acura/Honda Accord").
  {
    const pkgFit = extractPkgFitment(pkgText);
    if (pkgFit.length > 0) {
      const pkgModels = new Set();
      for (const row of pkgFit) {
        const ms = row.match(/\b[A-Z][A-Za-z0-9-]+/g) || [];
        for (const w of ms) {
          // Skip spec words so the whitelist is just Make/Model candidates.
          if (/^(L4|V6|V8|Front|Rear|Left|Right|Driver|Passenger|Petrol|Diesel|Coupe|Sedan|Hatchback|Convertible|Wagon|SUV|Truck)$/i.test(w)) continue;
          pkgModels.add(w);
        }
      }
      const sectionMentionsPkgModel =
        pkgModels.size > 0 &&
        fitList.some((r) => {
          for (const m of pkgModels) if (r.includes(m)) return true;
          return false;
        });
      if (pkgModels.size > 0 && !sectionMentionsPkgModel) {
        fitList = pkgFit;
      }
    }
  }
  ...
  // omitted (Type, OEM, Placement, …) so a partial model output still surfaces
  // real data — but never overwrite a field the model already provided. This
  // is what "just paste the supplier listing" should do: the package's own
  // part type / OEM numbers feed the Specifications table and the suggested
  // category, instead of being discarded when the model skips a section.
  let specificsFinal = specifics;
  {
    const fb = fallbackSpecifics(pkgText, fitment);
    const have = new Set(specificsFinal.map(([k]) => k));
    for (const [k, v] of fb) {
      if (!have.has(k)) specificsFinal.push([k, v]);
    }
  }
  ...
  // Two non-mutually-exclusive echoes can appear in sec[7]:
  //   (A) "ALL-echo" — the whole line is instruction text ("— verify ...").
  //       looksLikeCategoryEcho() returns true; we drop it entirely so
  //       inferCategoryPath() can recover a real path from Item Specifics.
  //   (B) "Hybrid" — "<real path> — verify ...". looksLikeCategoryEcho()
  //       ALSO returns true on the tail, but the leading real path is
  //       recoverable via stripCategoryEchoTail(). SKU 20-282P1 regression
  //       (Sept 2026): the LLM wrote "… > Power Steering Pumps — verify in
  //       the eBay Sell flow before publishing." — looksLikeCategoryEcho
  //       matched the tail, so the previous code threw away the entire
  //       string and fell through to inferCategoryPath(), which produced a
  //       slightly different path (losing the model-known canonical leaf).
  // Fix: try stripCategoryEchoTail() FIRST (handles hybrid by trimming the
  // tail); only fall through to inferCategoryPath() if the stripped result
  // is empty (i.e. the whole string was echo). looksLikeCategoryEcho() is
  // still consulted as a fast-path to detect "all echo" without trimming.
  let cleanedCategory = stripCategoryEchoTail(rawCategory);
  if (!cleanedCategory) cleanedCategory = ""; // was all echo
  const category =
    cleanedCategory ||
