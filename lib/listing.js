...
export function looksLikeCategoryEcho(s) {
  ...
  const t = s.trim();
  if (!t) return false;
  if (t.length > 120) return false; // a real path is long but not THAT long
...
export function buildResult(pkgText, llmText, model, seconds, tokens) {
  ...
  // else infer from Item Specifics[Type] (see inferCategoryPath). The
  // echo guard below drops the common case where the model regurgitates
  // the prompt's "— verify in the eBay Sell flow before publishing."
  // instruction as the entire section body — that's not a category, it's
  // instruction text masquerading as content.
  const rawCategory = para(sec[7] || "");
  const category =
    (rawCategory && !looksLikeCategoryEcho(rawCategory) ? rawCategory : "") ||
    inferCategoryPath(specificsFinal) ||
    inferCategoryPath(fallbackSpecifics(pkgText, fitment)) ||
    "-";
