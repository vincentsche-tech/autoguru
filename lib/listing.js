...
// usually means the input data package was empty). Industry pattern: <Part
// Type> for <Year-Range Make/Model> (<Placement>). Cap at 80 chars.
export function fallbackTitle(specifics, fitment) {
  const typeKV = (specifics || []).find(([k]) => k === "Type");
  const placeKV = (specifics || []).find(([k]) => k === "Placement on Vehicle");
  const type = typeKV ? typeKV[1] : null;
  const place = placeKV ? placeKV[1] : null;
  let year = "";
  const fm = (fitment || "").match(/\b(?:19|20)\d{2}\b/);
  if (fm) year = fm[0];
  let title = type || "Auto Part";
  if (year) title += ` for ${year}`;
...
export function buildResult(pkgText, llmText, model, seconds, tokens) {
  ...
  let titles = rawTitles.filter(isValidTitle).map((t) => ({
    text: t,
    len: t.length,
  }));
